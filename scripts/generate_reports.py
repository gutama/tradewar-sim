    ...
    try:
        with open(report_path, 'w') as f:
            ...
            f.write(f"This report analyzes the results of a trade war simulation with "
                   f"{len(df.columns[0]) if 'countries' in df.columns else len([col.split('_')[0] for col in df.columns if col.endswith('_gdp')])} countries over "
                   f"{df['year'].max() + 1 if 'year' in df.columns else 'multiple'} years.\n\n")
            ...
            for country, growth in policy_analysis["gdp_growth"].items():
                years = df['year'].max() + 1 if 'year' in df.columns else 5  # Default to 5 years if no year column
...
def generate_markdown_report(
    ...
    try:
        with open(report_path, 'w') as f:
            ...
            f.write("## Overview\n\n")
            f.write(f"This report analyzes the results of a trade war simulation with "
                   f"{len(df['countries'].iloc[0])} countries over "
                   f"{df['year'].max() + 1} years.\n\n")
